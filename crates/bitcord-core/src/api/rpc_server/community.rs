...
pub(super) fn register_community_methods(
    ...
    module.register_async_method("community_join", |params, ctx, _| async move {
        ...
        // Require a valid Ed25519 admin signature on all invites.
        // Unsigned invites are rejected to prevent forged community_id/public_key_hex substitution.
        if invite.sig_hex.is_empty() {
            return Err(invalid_params(
                "invalid invite: admin signature required; generate a new invite link from an admin account",
            ));
        }
        if pk_bytes != [0u8; 32] {
        ...
        let manifest = CommunityManifest {
            ...
            admin_ids: vec![],
            channel_ids: vec![],
            // Only keep the first seed node in the placeholder manifest.
            // The real admin-signed manifest (with the canonical seed list) will
            // replace this via gossip, but until then we don't want invite links
            // generated by freshly-joined members to re-propagate extra IPs that
            // may have been present in an old multi-address invite.
            seed_nodes: invite.seed_nodes.first().cloned().into_iter().collect(),
            version: 0,
            deleted: false,
        };
